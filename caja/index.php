...
if (file_exists($config_file)) {
    ...
    die("Error: No se encontró el archivo config.php en " . __DIR__ . ". Por favor, crea uno basado en config.php.example");
}
date_default_timezone_set('America/Argentina/Buenos_Aires');

// Fallback por si la constante no está definida en config.php
if (!defined('DB_TABLE_MOVIMIENTOS')) {
    define('DB_TABLE_MOVIMIENTOS', 'movimientos');
}
...
if ($is_logged_in && !isset($db_error)) {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'add_movimiento') {
        ...
        $stmt = $pdo->prepare("INSERT INTO " . DB_TABLE_MOVIMIENTOS . " (tipo, concepto, monto) VALUES (?, ?, ?)");
    ...
    if (isset($_GET['delete'])) {
        check_csrf_token();
        $stmt = $pdo->prepare("DELETE FROM " . DB_TABLE_MOVIMIENTOS . " WHERE id = ?");
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'edit_movimiento') {
        ...
        $stmt = $pdo->prepare("UPDATE " . DB_TABLE_MOVIMIENTOS . " SET tipo = ?, concepto = ?, monto = ?, fecha = ? WHERE id = ?");
    ...
    if (isset($_GET['export'])) {
        ...
        $saldo_acumulado = 0;
        if ($filter_date) {
            $stmt_prev = $pdo->prepare("SELECT SUM(CASE WHEN tipo = 'ingreso' THEN monto ELSE -monto END) as previo FROM " . DB_TABLE_MOVIMIENTOS . " WHERE DATE(fecha) < :fdate");
        ...
        $stmt = $pdo->prepare("SELECT * FROM " . DB_TABLE_MOVIMIENTOS . " $where_sql ORDER BY fecha ASC");
        $stmt->execute($params);
    ...
    $stmt = $pdo->prepare("SELECT 
        ...
        FROM " . DB_TABLE_MOVIMIENTOS . " $where_sql");
    $stmt->execute($params);
    $stats = $stmt->fetch();
    $saldo_total = $stats['saldo_total'] ?? 0;

    // Historial con filtro
    $stmt = $pdo->prepare("SELECT * FROM " . DB_TABLE_MOVIMIENTOS . " $where_sql ORDER BY fecha DESC LIMIT 100");
    $stmt->execute($params);
...
<body class="bg-gray-50 text-gray-900 min-h-screen">
    ...
    <?php else: ?>
        <!-- MAIN APP -->
        <nav id="caja-nav" class="bg-white shadow-sm border-b px-4 py-4 mb-8">
            <div class="max-w-4xl mx-auto flex justify-between items-center">
                <h1 class="text-xl font-bold text-blue-600">Caja Diaria</h1>
                <div class="flex items-center gap-4">
                    <a href="../index.php?logout=1" class="text-sm font-medium text-red-500 hover:text-red-700">Cerrar Sesión</a>
                </div>
            </div>
        </nav>
        <script>
            if (window.self !== window.top) {
                document.getElementById('caja-nav').style.display = 'none';
            }
        </script>
                                        ...
                                        <tr class="hover:bg-gray-50 transition">
                                            ...
                                            <td class="px-6 py-4 text-sm text-center">
                                                <div class="flex justify-center gap-2">
                                                    ...
                                                    <button onclick="showConfirm('Eliminar Movimiento', '¿Estás seguro de que deseas borrar este registro de caja? Esta acción no se puede deshacer.', () => window.location.href='?delete=<?php echo $mov['id']; ?>&csrf_token=<?php echo urlencode($_SESSION['csrf_token'] ?? ''); ?>')" class="text-red-500 hover:text-red-700 p-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
